...
def generate_reports(spark):
    ...
    # Считаем количество уникальных транзакций (продаж) на клиента, чтобы получить средний чек
    # Предположим, что каждая строка в fact_sales - это отдельная транзакция (чек)
    # Если несколько строк в fact_sales могут относиться к одному чеку, то нужна группировка по ID чека.
    # В нашей модели fact_sales каждая строка - это запись о продаже товара, возможно, в рамках одного чека.
    # Для "среднего чека" лучше иметь отдельный идентификатор транзакции (чека).
    # Если его нет, то можно считать среднюю сумму *одной записи о продаже* на клиента.
    # Или, если sale_total_price - это уже сумма всего чека (что маловероятно, если есть sale_quantity),
    # то нужно агрегировать иначе.
    # Пример ниже считает среднюю стоимость ОДНОЙ СТРОКИ в fact_sales для клиента.
    # Для более точного среднего чека, если несколько строк в fact_sales это один чек,
    # нам нужен был бы ID чека.
    # (sum(total_price) / count(quantity)) - это скорее средняя цена *за единицу товара* по клиенту, если quantity > 1.
    # sum(total_price) / count(distinct transaction_id) если бы он был.
    # Так как его нет, "sale_quantity" как "количество чеков/транзакций".
    # НО! sale_quantity в fact_sales - это количество товара в транзакции.
    # Если мы хотим средний чек, то нужно количество УНИКАЛЬНЫХ транзакций.
    # Если каждая строка в fact_sales - это уникальная транзакция, то count(*) или count("sale_sk")
